...
import requestManager, { defaultOptions } from './index';
...
describe('requestManager', () => {
  ...
  it('fetches successfully data from an API', async () => {
    ...
    await expect(requestManager('POST', endPoint)).resolves.toEqual(
      responseData.data
    );
  ...
  it('fetches erroneously data from an API', async () => {
    ...
    await expect(requestManager('POST', endPoint)).rejects.toThrow(
      errorMessage
    );
